...
export class StationListComponent implements OnInit {
  ...
  filterStations(): void {
    ...
    // Set the `updatedPosition` to reflect positions in the actual list (`stations`)
  ...
  onDrop(event: CdkDragDrop<any[]>): void {
    // Handle drag-and-drop only on the actual `stations` list
    const draggedStationId = this.filteredStations[event.previousIndex]['Station Id'];
  
    // Find the current index of the dragged station in the `stations` list
    const draggedIndex = this.stations.findIndex(
      (station) => station['Station Id'] === draggedStationId
    );
  
    // Find the target station's ID in `filteredStations`
    const targetStationId = this.filteredStations[event.currentIndex]['Station Id'];
  
    // Find the target index in the `stations` list
    const targetIndex = this.stations.findIndex(
      (station) => station['Station Id'] === targetStationId
    );
  
    // Reorder the `stations` list
    const [draggedStation] = this.stations.splice(draggedIndex, 1);
    this.stations.splice(targetIndex, 0, draggedStation);
  
    // Reapply filters and synchronize positions
    this.filterStations(); // Refresh `filteredStations`
    this.updatePositionsInFilteredStations(); // Synchronize positions
  }
...
getStationCurrentPosition(stationId: string): number {
  // Find the station's position in the actual list (`stations`)
...
updateStationPosition(station: any): void {
  ...
  // Find the current index in the main list
  ...
  // Insert the station at the new position
  this.stations.splice(station.updatedPosition - 1, 0, movedStation);

  // Refresh both filteredStations and updated positions
  this.filterStations(); // Update the filtered list based on search/filter
  this.updatePositionsInFilteredStations(); // Synchronize positions
  alert('Position updated successfully!');
}
updatePositionsInFilteredStations(): void {
  this.filteredStations.forEach((station) => {
    // Update the `updatedPosition` to reflect the position in the main `stations` list
    station.updatedPosition = this.getStationMainPosition(station['Station Id']);
  });
}
...
getStationMainPosition(stationId: string): number {
  // Find the station's position in the main list
  return this.stations.findIndex((station) => station['Station Id'] === stationId) + 1;
}
